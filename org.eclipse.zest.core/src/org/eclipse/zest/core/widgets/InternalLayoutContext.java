...
class InternalLayoutContext implements LayoutContext {
	...
	public ConnectionLayout[] getConnections(EntityLayout source, EntityLayout target) {
		...
		Set<EntityLayout> targets = new HashSet<>();
		...
		for (NodeLayout layout : sourcesList) {
			ConnectionLayout[] outgoingConnections = layout.getOutgoingConnections();
			for (ConnectionLayout connection : outgoingConnections) {
				if ((connection.getSource() == layout && targets.contains(connection.getTarget()))
						|| (connection.getTarget() == layout && targets.contains(connection.getSource()))) {
